	...
	uint32_t seq, uint8_t hlen, uint8_t flags)
{
	...
	else if ((flags & TCP_FLAG_RST) == 0 && tcb->rcv.ts != 0)
	...
	uint32_t pid, uint32_t swcsm)
{
	...
	// QZ: Fixing inconsistency between header space allocation and usage
	else if ((flags & TCP_FLAG_RST) == 0 && s->tcb.rcv.ts != 0)
	// else if ((flags & TCP_FLAG_RST) == 0 && s->tcb.so.ts.raw != 0)
	...
	if (likely(s->s.type == TLE_V4)) {
		...
		*/
		if (likely(ol_flags & PKT_TX_TCP_CKSUM)) {
			l4h->cksum = 0;
			l3h->hdr_checksum = 0;
		} else if (swcsm) {
			l4h->cksum = _ipv4_udptcp_mbuf_cksum(m, len, l3h);
...
calc_smss(uint16_t mss, const struct tle_dest *dst)
{
	...
	//
	// QZ: fix MSS with header len calculation
	//
	//
	// mss = RTE_MIN(n, mss);
	// return mss;
	...
	uint32_t flags)
{
	...
	// QZ: offload cksum for control packets as well
	// rc = tcp_fill_mbuf(m, s, dst, 0, s->s.port, seq, flags, pid, 1);
	rc = tcp_fill_mbuf(m, s, dst, dst->ol_flags, s->s.port, seq, flags, pid, 1);
	if (rc == 0)
	...
	const union seg_info *si, uint32_t ts, struct rte_mbuf *m)
{
	...
	rc = tcp_fill_mbuf(m, s, &dst, dst.ol_flags, pi->port, seq,
	...
	struct rte_mbuf *rp[], int32_t rc[], uint16_t num)
{
	...
	for (i = 0; i != num; i++) {
		...
		t = pi[i].tf.type;
		// QZ: remove the verification of cksum as it's offloaded to hardware
		// pi[i].csf = check_pkt_csum(pkt[i], pi[i].csf, t, IPPROTO_TCP);
		pi[i].csf = 0;
		stu.t[t] = mt;
	}

	if (likely(stu.t[TLE_V4] != 0))
		stbl_lock(st, TLE_V4);
	if (stu.t[TLE_V6] != 0)
		stbl_lock(st, TLE_V6);
	...
	for (i = 0; i != num; i += j) {
		...
		// QZ: SYN packet might have other TCP flags, except the case of SYN+ACK
		// } else if (pi[i].tf.flags == TCP_FLAG_SYN) {
		} else if ((pi[i].tf.flags & TCP_FLAG_SYN) && !(pi[i].tf.flags & TCP_FLAG_ACK)) {
...
uint16_t
tle_tcp_stream_send_no_free(struct tle_stream *ts, struct rte_mbuf *pkt[], uint16_t num)
{
	uint32_t i, j, k, mss, n, state;
	int32_t rc;
	uint64_t ol_flags;
