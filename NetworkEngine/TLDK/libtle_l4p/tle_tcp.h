...
/**
 * QZ: get if a tcp stream is established.
 * @param s
 *   Pointer to the stream.
 * @return
 *   1 - established.
 ...
 */
int
tle_tcp_stream_get_established(const struct tle_stream *s);
...
/**
 * Same as tle_tcp_stream_send except pkt will not be freed
 * @param s
 *   TCP stream to send packets over.
 * @param pkt
 *   The burst of output packets that need to be send.
 ...
 *   - ENOTCONN - the stream is not connected.
 */
uint16_t tle_tcp_stream_send_no_free(struct tle_stream *s, struct rte_mbuf *pkt[],
	uint16_t num);
