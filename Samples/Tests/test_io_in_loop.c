...
void firstio_callback_func(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg) {
    struct mythread_ctx *myctx = cb_arg;
    SPDK_NOTICELOG("first IO Callback running...\n");
    int i = 0;
    while (i < 5) {
    ...
    spdk_app_stop(0);
    SPDK_NOTICELOG("app stop returned\n");
}
...
void TestBackEnd(
    ...
    SPDK_NOTICELOG("DOING VERY FIRST io...\n");
    void *tmpbuf = malloc(4096);
    int rc = bdev_read(myctx->SPDKContext, tmpbuf, 0, 1024, firstio_callback_func, myctx);
    SPDK_NOTICELOG("bdev_read() rc: %d\n", rc);
    SPDK_NOTICELOG("FINISHED VERY FIRST IO ..., status ptr: %p, value: %hu\n", &(myctx->cookie), myctx->cookie);
