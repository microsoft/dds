...
void RequestProducer(
	...
	for (size_t r = 0; r != NumRequests; r++) {
		while (InsertToRequestBuffer(RingBuffer, Requests[r]->Data, Requests[r]->Size + sizeof(int)) == false) {
			this_thread::yield();
		}
	}
}
...
void RequestConsumer(
	...
	while (numReqProcessed != TotalNumRequests) {
		while(FetchFromRequestBuffer(RingBuffer, pagesOfRequests, &remainingSize) == false) {
			this_thread::yield();
		}
		...
		while (true) {
			...
			// cout << "Number of ints = " << numInts << endl;
			...
			/*
			if (numReqProcessed % 1000 == 0) {
				cout << "Progress = " << numReqProcessed << endl;
			}
			*/
...
void EvaluateRequestRingBufferNoConcurrency() {
	...
	const size_t requestsPerProducer = 10000;
	...
	ringBuffer = AllocateRequestBuffer(Buffer);
	...
	DeallocateRequestBuffer(ringBuffer);
