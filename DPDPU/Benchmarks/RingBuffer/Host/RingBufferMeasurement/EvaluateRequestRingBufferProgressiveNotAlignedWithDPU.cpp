...
void RequestProducerWithDPU(
	...
	for (size_t r = 0; r != NumRequests; r++) {
		while (InsertToRequestBufferProgressiveNotAligned(RingBuffer, Requests[r]->Data, Requests[r]->Size + sizeof(int)) == false) {
			this_thread::yield();
		}
	}
	while (!CheckForCompletionProgressiveNotAligned(RingBuffer)) {
		this_thread::yield();
	}
	profiler.Stop();
...
void EvaluateRequestRingBufferProgressiveNotAlignedWithDPU() {
	...
	RequestRingBufferProgressiveNotAligned* ringBuffer = NULL;
	...
	ringBuffer = AllocateRequestBufferProgressiveNotAligned(buffer);
